...
export default function OnCampusFinancesPage() {
  ...
  const allPayments = useMemo(() => {
    const onCampusRooms = mockRooms.filter(room => 
  ...
  const onCampusProperties = mockProperties.filter(p => p.type === 'lodge')
  ...
  const handleExport = () => {
    ...
    a.href = url
    a.download = `on-campus-finances-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
  }
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 lg:p-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">On-Campus Finances</h1>
          <p className="text-gray-600">Financial overview for on-campus properties</p>
        </div>
